...
此实例是我根据onos-app-samples中oneping修改的。onos-app-smaples项目中采用maven进行编译，而当前版本的onos采用bazel编译，另外项目中使用的部分包名有变动。

## 要求
根据onos开发环境搭建文档搭建onos开发环境，并将nons项目源代码导入到IntelliJ中。

## 创建项目

### 创建需要开发的包的文件结构
在IDE中，在apps上右键->New->Bazel Package新建包并命名为oneping。将自动创建的BUILD.bazel修改为BUILD，仿照项目中其他的app文件结构，在oneping目录下创建文件结构，如下所示
```
.
├── BUILD
└── src
...
onos_app(
    ...
    description = "allow one ping in one minute",
    title = "One ping demo",
    url = "http://www.sgkind.com",
)
```

### 修改package-info.java文件
将下面的代码写入到package-info.java文件
```
/*
...
package com.sgkind.oneping;
```

注：
  必须要license信息，否则会报错，上面是复制apps中其他程序的。

### 修改OnePing.java内容
将下面的代码复制到OnePing.java文件中
```java
/*
...
### 更新项目的.bazelproject文件
采用下面的命令更新.bazelproject
```bash
$ onos-gen-bazel-project > $ONOS_ROOT/.ijwb/.bazelproject
```

如果找不到onos-gen-bazel-project命令，则可以
```bash
...
### re-sync项目
在IDE中，以此选择Bazel->sync->Sync project with BUILD，等待IDE执行完成。编译好的包的路径为`$ONOS_ROOT/bazel-bin/apps/oneping`
...
#### 安装编译的包
```bash
$ cd $ONOS_ROOT/bazel-bin/apps/oneping/
$ onos-app localhost install onos-apps-oneping-oar.oar
{"name":"org.onosproject.oneping","id":191,"version":"2.4.0.SNAPSHOT","category":"Traffic Engineering","description":"oneping test","readme":"oneping test","origin":"ONOS Community","url":"http://www.sgkind.com","featuresRepo":"mvn:org.onosproject/onos-apps-oneping/2.4.0-SNAPSHOT/xml/features","state":"INSTALLED","features":["onos-apps-oneping"],"permissions":[],"requiredApps":[]}
```
...
```bash
$ onos localhost
Enter passphrase for key '/home/sgk/.ssh/id_rsa': 
...
#### 在mininet中创建拓扑
```bash
$ sudo mn --topo=single,2 --controller=remote,ip=192.168.0.7
*** Creating network
*** Adding controller
Connecting to remote controller at 192.168.0.7:6653
*** Adding hosts:
h1 h2 
*** Adding switches:
s1 
...
因为当前没有激活onos中的fwd模块，故节点之间不能联通

#### 激活onos中的fwd模块
在执行`onos local`命令并连接到onos的终端中，执行

```bash
sgk@root > app app activate org.onosproject.fwd                             16:24:00
Activated org.onosproject.fwd
```

此时回到mininet中，查看各节点之间是否相通
```bash
mininet> pingall
*** Ping: testing ping reachability
h1 -> h2 
...
64 bytes from 10.0.0.2: icmp_seq=1 ttl=64 time=0.576 ms
...
rtt min/avg/max/mdev = 0.075/0.245/0.576/0.234 ms
```
...
在执行`onos local`命令并连接到onos的终端中，执行
```bash
sgk@root > app activate org.onosproject.oneping                         16:29:17
Activated org.onosproject.oneping
```

注：
   在代码中指定的包为`com.sgkind.oneping`，此处为什么变为`org.onosproject.oneping`还未知
...
--- 10.0.0.2 ping statistics ---
3 packets transmitted, 1 received, 66% packet loss, time 2021ms
rtt min/avg/max/mdev = 18.990/18.990/18.990/0.000 ms
```

可以看到ping发送的三个包只有一个对方收到了，另外两个没有收到。说明模块能正常运行。